    ...
    if (isset($_FILES['pp']['name']) && !empty($_FILES['pp']['name'])) {
        ...
        if ($error === 0) {
            ...
            if (in_array($img_ex_to_lc, $allowed_exs)) {
                ...
                do {
                    if (empty($nom) || empty($prenom) || empty($email) || empty($portofolio) || empty($bio)) {
                        $errorMessage = "Tous les champs sont requis";
                        break;
                    }

                    $stmt = $conn->prepare("INSERT INTO formateur (nom, prenom, email, portofolio, photo_profile, biographie) VALUES (?, ?, ?, ?, ?, ?)");
                    $stmt->bind_param("ssssss", $nom, $prenom, $email, $portofolio, $new_img_name, $bio);
                    $result = $stmt->execute();
                ...
                $errorMessage = "Format de fichier non autorisé. Seuls les fichiers jpg, jpeg, png sont acceptés.";
            }
        } else {
